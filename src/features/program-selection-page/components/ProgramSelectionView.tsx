...
export function ProgramSelectionView({
  ...
  const levels = useMemo(
    () =>
      data
        ? Array.from(new Set(data.programs.map((program) => program.level)))
        : [],
    [data],
  );
  ...
  const visiblePrograms = useMemo(
    () =>
      data?.programs.filter(
        (program: StudyProgram) => program.level === activeLevel,
      ) ?? [],
    [activeLevel, data],
  );
  ...
  const baseSelection = useMemo(() => {
    ...
    return {
      ...
      admissionPathId: "",
      admissionPathName: "",
      admissionPathDescription: "",
      waveId: "wave-1",
      waveName: data.waveName,
      registrationFee: data.registrationFee,
    } satisfies StudySelection;
  }, [data, programId]);

  const campuses = useMemo(
    () =>
      data
        ? Array.from(new Set(data.classSchedules.map((item) => item.campus)))
        : [],
    [data],
  );
  ...
  const availableStudySystems = useMemo(
    () =>
      data?.classSchedules.filter(
        (item: ClassSchedule) => item.campus === selectedCampus,
      ) ?? [],
    [data, selectedCampus],
  );

  const selectedStudySystem = availableStudySystems.find(
    (system: ClassSchedule) => system.session === selectedSession,
  );

  const selectedAdmissionPath = data?.admissionPaths.find(
    (path: AdmissionPath) => path.id === selectedAdmissionPathId,
  );
  ...
  useEffect(() => {
    ...
    if (selectedCampus && selectedSession === "") {
      const fallback = data.classSchedules.find(
        (system: ClassSchedule) => system.campus === selectedCampus,
      );
    ...
    if (baseSelection?.level) {
      setSelectedLevel(baseSelection.level);
    }
  }, [baseSelection?.level, campuses, data, selectedCampus, selectedSession]);
